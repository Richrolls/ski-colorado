...
@router.post("/api/resorts/{resort_id}/comments", response_model=Comment)
async def create_comment(
    resort_id: str,
    repo: CommentQueries = Depends(),
    account_data: dict = Depends(authenticator.get_current_account_data),
):
    comment = repo.create(resort_id=resort_id, user_id=account_data['id'])
    return comment
...
@router.get("/api/resorts/{resort_id}/comments/{comment_id}", response_model=Comment)
